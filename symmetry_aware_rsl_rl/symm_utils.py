...
def configure_observation_space_representations(
    ...
    try:
        ...
        raise ImportError(
            "morpho_symm package is required to configure observation group representations"
        ) from e

    G = load_symmetric_system(
        robot_name=robot_name, return_robot=False, joint_space_order=joints_order
    )
    rep_Q_js = G.representations[
        "Q_js"
    ]  # Representation on joint space position coordinates
    rep_TqQ_js = G.representations[
        "TqQ_js"
    ]  # Representation on joint space velocity coordinates
    rep_Rd = G.representations["R3"]  # Representation on vectors in R^d
    rep_Rd_pseudo = G.representations[
        "R3_pseudo"
    ]  # Representation on pseudo vectors in R^d
    ...
    rep_Rd_on_limbs = rep_kin_three.tensor(
        rep_Rd
    )  # Representation on signals R^d on the limbs
    rep_Rd_on_limbs.name = "Rd_on_limbs"
    rep_Rd_pseudo_on_limbs = rep_kin_three.tensor(
        rep_Rd_pseudo
    )  # Representation on pseudo vect R^d on the limbs
    ...
    rep_z = escnn_representation_form_mapping(
        G, {g: rep_Rd(g)[2:3, 2:3] for g in G.elements}, name="base_z"
    )
    ...
    for obs_name in obs_names:
        ...
        if obs_name == "qpos":
            ...
            obs_reps[obs_name] = (
                rep_Rd + rep_Rd_pseudo + rep_TqQ_js
            )  # lin_vel , ang_vel, joint_vel
            ...
            "base_lin_vel" in obs_name or "base_lin_acc" in obs_name
        ):  # base_lin_vel / base_lin_vel:base (base frame)
            obs_reps[obs_name] = rep_Rd
            ...
            "feet_pos" in obs_name or "feet_vel" in obs_name
        ):  # feet_pos:frame := feet_pos:world or feet_pos:base
